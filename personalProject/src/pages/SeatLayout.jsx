...
const {axios
      , getToken,user} = useAppContext(); 

const getShow = async()=>{
  try {
    const { data } = await axios.get(`/api/show/all/${id}`)
   if(data.success){
  setShow({
  movie: data.movie,
  dateTime: data.dateTime,
  casts: data.movie.casts,
...
const renderSeats = (row,count=9)=>(
  <div key={row} className="flex gap-2 mt-2">
    <div className='flex flex-wrap items-center justofy-center gap-2'>
...
const bookTickets = async () =>{
  try {
    if(!user) return toast.error('Please login to proceed')
      ...
      if(!selectedTime?.showId || !selectedSeats.length) return toast.error('Please select a time and seats');
  ...
  return show? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      <BlurCircle bottom="0px" left="0px" />
       <h1 className='text-2xl font-semibold mb-4'>Select Your Select</h1>
